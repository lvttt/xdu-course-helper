...
const mockPublicInfo = {
    ...
    labelMap: {
        ...
        choicedCourseZxfCanNotMoreThanKclbRequireHighestXf:
            '已选的学分不能超过课程类别（(kclbmc)）要求的最高学分（已选(yxzxf)学分，要求最高(zgxf)学分）',
        dwkfgxrlym:
            '本课程对外开放共享选课容量为(rssx)，当前开放共享的名额已经选课(yxrs)人。容量已满！\n',
        haveCourses: '已选课程',
        currentLcOpenedCourses: '本轮次开课课程查询',
        choicedCourseZxfCanNotMoreThanProgramRequireHighestXf:
            '已选的总学分不能超过培养方案要求的最高学分（已选(yxzxf)学分，要求最高(zgxf)学分）',
        currentCoursesAreNotSelectedForThePlan:
            '你的培养计划还未选择当前课程或你当前计划的状态不满足可选状态要求',
        ...
        choiceCourseXfCanNotMoreThanUpperLimit:
            '选择的课程的学分不能超过设置的学分上限（已选总学分(yxzxf)，当前课程学分(dqkcxf)，上限(xfsx)学分）',
        ...
        choicedCourseZmsCanNotMoreThanKclbRequireHighestMs:
            '已选的门数不能超过课程类别（(kclbmc)）要求的最多门数（已选(yxms)门，要求最多(zdms)门）',
        ...
        choicedAllGxkXfCanNotMoreThanUpperLimit:
            '选择的所有公选课的学分不能超过设置的学分上限（已选总学分(yxzxf)，当前课程学分(dqkcxf)，上限(xfsx)学分）',
        ...
        choicedAllGxkMsCanNotMoreThanUpperLimit:
            '选择的所有公选课的门数不能超过设置的门数上限（已选门数(yxms)，上限(mssx)门）',
        hasArrearageCanNotChoiceCourse: '存在欠费信息，不可选课',
        dqczdkcbmztxyq: '当前操作的课程不满足退选要求',
        pageExpired: '页面已过期，请刷新页面后重试',
        forbidChoiceCourse: '禁止选课',
        choiceCourseMsCanNotMoreThanUpperLimit:
            '选择的课程的门数不能超过设置的门数上限（已选门数(yxms)，上限(mssx)门）',
        ...
        youHasApplyMxCurrentCoursePleaseNotChoiceCurrentCourse:
            '你已申请免修当前课程，请不要对申请免修的课程进行选课操作',
        ...
        choicedCourseZmsCanNotMoreThanProgramRequireHighestMs:
            '已选的门数不能超过培养方案要求的最多门数（已选(yxms)门，要求最多(zdms)门）',
        choicedCourseZxfCanNotMoreThanDxzRequireHighestXf:
            '已选的学分不能超过课程组（(dxzmc)）要求的最高学分（已选(yxzxf)学分，要求最高(zgxf)学分）',
        youHaveAlreadyTakenThisCourse: '您已选过该课程，不能再次选择',
        unFinishedCheckin: '未完成报到',
        openedCourses: '开课课程查询',
        choicedCourseZmsCanNotMoreThanDxzRequireHighestMs:
            '已选的门数不能超过课程组（(dxzmc)）要求的最多门数（已选(yxms)门，要求最多(zdms)门）',
    ...
    school_id: '10701',
    gotoWdjhPath:
        'http://yjsxk.xidian.edu.cn/yjsxkapp/sys/wdpyjhapp/*default/index.do?EMAP_LANG=zh',
    xsMap: {
    ...
    xtcsMap: {
        ...
        xkgl_xsxkadminloginurl:
            'http://ehall.xidian.edu.cn/yjsxkapp/sys/xsxkapp/login/adminLogin.do',
        ...
        xkgl_yjskcjxdgurl_shtech:
            'http://localhost:8080/emap/sys/kcglapp/public/jxdg_shtech/info.html?bjdm={{bjdm}}',
        xkgl_xghkclb: '',
        xkgl_tksfljsfrl: '1',
        ...
        pyjh_kckkxqwhxs: '2',
        xkgl_gldcxmdsfzrl: '1',
        xkgl_bkkcjxdgurl_shtech:
            'https://eams.shanghaitech.edu.cn/eams/syllabusInfo!syllabusInfo.action?strCourseNo={{kcdm}}&strSemester={{xnxq}}',
        xkgl_xxfsfs: '3',
