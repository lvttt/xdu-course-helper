                    ...
                    >
                        ...
                        <template v-if="extConfig.rememberMe" #default>
                            ...
                            >
                                ...
                                <div
                                    v-if="loginResponse"
                                    class="mt-2 p-2 bg-blue-50 border border-blue-100 rounded text-[11px] text-blue-800 break-all"
                                >
...
const checkUrl = async () => {
    ...
    isSupportedSite.value =
        tab?.url?.includes('yjsxk.xidian.edu.cn/yjsxkapp/sys/xsxkapp/') || false;
};
...
const recognizeCaptcha = async (imageUrl) => {
    try {
        const response = await fetch(
            `https://api.vitphp.cn/Yzcode/?img=${encodeURIComponent(imageUrl)}`
        );
